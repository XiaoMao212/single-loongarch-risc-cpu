...
assign {csr_re, csr_we, csr_wmask, csr_wvalue, csr_num, ds_ex, es_ertn, 
        es_adef, ds_wrong_addr, ds_ecode, es_esubcode} = ds_exception;
...
assign es_wrong_addr = es_adef ? ds_wrong_addr : alu_result;//传递adef指令是为了方便判断wrong_addr，后续csr可以通过一级编码二级编码判断adef异常，所以不需要再传这个信号了
...
assign es_ready_go    = es_reflush?  1:
                        es_need_mem ? (data_sram_en && data_sram_addr_ok):
                        inst_is_div ? (dout_tvalid | doutu_tvalid) : es_valid;     
assign data_sram_en    = ms_allowin && es_need_mem;
assign data_sram_we    = es_mem_we && (es_valid & ~ms_ex & ~es_reflush & ~st_ale) ? 
                        (es_store_op == 2'b01 ? st_b : (es_store_op == 2'b10 ? st_h : 4'hf)) : 4'h0;
assign data_sram_addr  = alu_result;
assign data_sram_wdata = es_store_op == 2'b01 ? st_b_result :
