...
//这里没有用串行化思维，指令手册给出的TLB查找流程是用串行化思维取实现，我们用并行化实现，同时比较所有项，
genvar i;
generate for(i = 0; i < TLBNUM; i = i + 1) begin
    assign match0[i] = (s0_vppn[18:9] == tlb_vppn[i][18:9])
                    && (tlb_ps4MB[i] || s0_vppn[8:0] == tlb_vppn[i][8:0])//4MB只需要比较高10位虚拟页号，4KB还需要比较低九位
...
generate for(i = 0; i < TLBNUM; i = i + 1) begin
        ...
        assign cond[i][2] = s1_asid == tlb_asid[i];//invtlb传进来的vppn和asid复用访存路径
        assign cond[i][3] = (s1_vppn[18:10] == tlb_vppn[i][18:10]) && (tlb_ps4MB[i] || s1_vppn[9:0] == tlb_vppn[i][9:0]);

end    
endgenerate

assign invtlb_mask[0] = 16'hffff;           // op = 0, clear all
assign invtlb_mask[1] = 16'hffff;           // op = 1, same as op = 0
/*
assign invtlb_mask[2] = cond[1];            // op = 2, clear G == 1
assign invtlb_mask[3] = cond[0];            // op = 3, clear G == 0
assign invtlb_mask[4] = cond[0] & cond[2];  // op = 4, clear G == 0 and ASID match
assign invtlb_mask[5] = cond[0] & cond[2] & cond[3];    // op = 5, clear G == 0 and ASID match and VA match
assign invtlb_mask[6] = (cond[1] | cond[2]) & cond[3];  // op = 6, clear G == 1 or ASID match and VA match
*/

generate for(i = 0; i < TLBNUM; i = i + 1) begin
    assign invtlb_mask[2][i] = cond[i][1];
    assign invtlb_mask[3][i] = cond[i][0];
    assign invtlb_mask[4][i] = cond[i][0] & cond[i][2];
    assign invtlb_mask[5][i] = cond[i][0] & cond[i][2] & cond[i][3];
    assign invtlb_mask[6][i] = (cond[i][1] | cond[i][2]) & cond[i][3];
end endgenerate
