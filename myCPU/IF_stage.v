...
assign to_fs_valid     = ~reset & pre_if_ready_go & ~(pre_fs_reflush | fs_reflush);//收到异常信号时不管什么情况都设置to_fs_valid无效，直到这个错误的pc已经在IF阶段接收到data_ok
...
always @(posedge clk) begin
    if(reset)
        fs_inst_cancel <= 1'b0;
    else if((fs_reflush | br_taken & ~br_stall) && ~fs_allowin && ~fs_ready_go)//异常取消时，上一条指令data_ok=0，说明异常发生时（data_ok=1,即使ds_allowin=0，reg也会暂存这条指令），会执行这条不该执行的指令
        fs_inst_cancel <= 1'b1;
...
assign fs_to_ds_bus = {fs_adef_data, fs_inst, fs_pc};
...
//wire fs_inst_cancel; 
//wire fs_inst_cancel_ds;
//assign fs_inst_cancel = fs_reflush | fs_reflush_reg|br_taken & ~br_stall;
//assign fs_inst_cancel_ds = (fs_reflush | fs_reflush_reg|br_taken & ~br_stall)& ~fs_ready_go & ds_allowin;
