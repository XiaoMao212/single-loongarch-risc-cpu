...
always @(posedge clk) begin
    ...
    else if(writebuf_next_state == WRITEBUF_IDLE) begin
        write_strb  <= 1'b0;
    end
end
always @(posedge clk) begin
...
always @(posedge clk) begin
    if(reset) begin
        ...
        else if(replace_way == 1'b0 && reg_op == 1'b0)
...
always @(posedge clk) begin
    if(reset) begin
        ...
        else if(replace_way == 1'b1 && reg_op == 1'b0)
...
always @(posedge clk) begin
    if (wr_req && wr_rdy) begin
        // 判断是不是写UART（根据你的UART地址）
        if (wr_addr == 32'hbfe001e0) begin
            $display("TIME %0t: UART WRITE addr=%h data=%h wstrb=%b", 
                     $time, wr_addr, wr_data, wr_wstrb);
        end
    end
end                   
